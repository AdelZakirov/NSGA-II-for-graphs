...
function child = mutationRand( parent, opt, state, fraction, M)
    if (rand(1)> (1-opt.mutation{1,2}) && M > length(parent))
        nodes = 1:M;
        nodes(parent) = []; % nodes that are not in chromosome
        N = length(parent);
        M1 = length(nodes);
        ...
        mutation = randi([1,M1],1,1);
        % mutate
        mutatedParent = parent;
        mutatedParent(mutationPoint) = nodes(mutation);%parent(:,mutationPoint) + mutation;
        child = mutatedParent;
    else
        child = parent;
    end
