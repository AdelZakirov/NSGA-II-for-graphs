...
for k = 1:length(pop)
    if(pop(k).rank == 1)
        pareto1 = [pareto1; pop(k).obj];
    elseif (pop(k).rank == 2)
        pareto2 = [pareto2; pop(k).obj];
    elseif (pop(k).rank == 3)
        pareto3 = [pareto3; pop(k).obj];
    end
    
end
pareto1 = sortrows(pareto1);
pareto2 = sortrows(pareto2);
pareto3 = sortrows(pareto3);
...
%     hold on
%     plot(pareto2(:,1), pareto2(:,2), 'xr-');
%     plot(pareto3(:,1), pareto3(:,2), 'xg-');
%     hold off
